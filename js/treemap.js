(function(){
        ...
        /* Hook into class-select and receptor toggle changes to update treemap.
           Because treemap.js loads before app.js, we cannot monkey-patch
           toggleReceptor directly (app.js would overwrite the wrapper).
           Instead, use event delegation on the receptor-toggles container. */
        document.getElementById('receptor-toggles').addEventListener('click', function(e) {
          var btn = e.target.closest('.receptor-toggle-btn');
          if (!btn) return;
          // toggleReceptor in app.js handles the Set + bar chart update;
          // we just need to refresh the treemap when it is visible.
          setTimeout(function() {
