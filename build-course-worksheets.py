...
# Module 8 — EMDR for Trauma & PTSD
MODULES.append(dict(file="emdr-trauma-worksheets.pdf",
    title="Module 8 — EMDR Therapy for Trauma & PTSD",
    subtitle="Target sequence · Phase 2 stabilization · Phase 3 assessment · reprocessing log · outcomes",
    worksheets=[
        dict(name="Three-Pronged Target Sequence Plan (Phase 1)",
             instruction="Map past events (touchstone), present triggers, and future templates. SUDS = 0-10. Use floatback to find touchstone memories.",
             elements=[("fields", [("Presenting problem / symptom", 1), ("Core negative cognition / theme", 1), ("Desired positive cognition", 1)]),
                       ("note", "PAST — touchstone / formative memories"),
                       ("table", ["Memory / event", "Age", "SUDS 0-10", "Process order"], [1.8, 0.5, 0.7, 0.8], 6),
                       ("note", "PRESENT — current triggers"),
                       ("table", ["Current trigger / situation", "Reaction (emotion / behavior)", "SUDS 0-10"], [1.5, 1.4, 0.7], 5),
                       ("note", "FUTURE — templates"),
                       ("table", ["Anticipated situation", "Desired adaptive response (with PC)"], [1.2, 1.6], 4)]),
        dict(name="Phase 2 — Preparation & Resourcing",
             instruction="Confirm readiness and install affect-regulation resources before any reprocessing. Enhance resources with short, slow BLS sets.",
             elements=[("note", "Readiness & dissociation screen"),
                       ("checklist", ["Rapport & informed consent for EMDR obtained",
                                      "Model, mechanics & metaphor explained (\"just notice\")",
                                      "Stop signal agreed and practiced",
                                      "Dual attention tested (hold image + BLS, return to now)",
                                      "Affect tolerance adequate; client can down-regulate",
                                      "Dissociation screened (e.g., DES-II); complex presentation flagged",
                                      "Current safety: no active suicidality / unmanaged substance use / danger",
                                      "Support system & between-session coping in place"]),
                       ("note", "Calm / Safe Place"),
                       ("fields", [("Image / place (real or imagined)", 1), ("Sights, sounds, smells, sensations", 1),
                                   ("Emotion & where felt in the body", 1), ("Cue word", 0),
                                   ("Any disturbance that arose with BLS? (if yes, pick another)", 1)]),
                       ("note", "Container"),
                       ("fields", [("The container (what it looks like, how it seals)", 1), ("What will be placed in it between sessions", 1)]),
                       ("note", "Resource Development & Installation (RDI)"),
                       ("table", ["Resource", "Type (mastery / relational / symbolic)", "Felt sense / cue word"], [1.3, 1.3, 1.1], 4)]),
        dict(name="Phase 3 — Target Assessment",
             instruction="Fully activate one target: image, NC, PC (same theme), VOC (1-7), emotion, SUDS (0-10), body location.",
             elements=[("fields", [("Target memory", 1), ("Image (\"worst part\")", 1),
                                   ("Negative Cognition (present-tense \"I am…\")", 1),
                                   ("Positive Cognition (adaptive, same theme)", 1),
                                   ("VOC baseline (1-7) ______     Emotion ________________________", 0),
                                   ("SUDS baseline (0-10) ______   Body location ________________________", 0)])]),
        dict(name="Reprocessing Log (Phases 4-6)",
             instruction="Log each BLS set and what emerged; track SUDS down to 0 (desensitization), then VOC up to 7 (installation), then a clear body scan.",
             elements=[("note", "Desensitization sets (Phase 4)"),
                       ("table", ["#", "What came up (image / thought / emotion / sensation)", "Interweave (if stalled)", "SUDS 0-10"],
                        [0.4, 2.0, 1.1, 0.7], 8),
                       ("note", "Installation & body scan (Phases 5-6)"),
                       ("fields", [("Positive Cognition installed (confirm it still fits)", 1),
                                   ("VOC before ______ → after ______ (aim 7)", 0),
                                   ("Body scan result (clear? residual sensation reprocessed?)", 1)])]),
        dict(name="Goals & Outcomes",
             instruction="Set a functional goal and track a repeated PTSD measure (PCL-5 total 0-80; ~10-20 point drop = clinically meaningful).",
             elements=[("smart",),
